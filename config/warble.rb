...
#-------------------------#
# if want to generate .war file should write this 'bundle exec warble'
...
Warbler::Config.new do |config|
  ...
  config.jar_name = "ror_JRuby"

  # ระบุ Main-Class สำหรับ JRuby on Rails
  config.manifest_file = "META-INF/MANIFEST.MF"
  
  # รวมโฟลเดอร์ META-INF
  config.dirs += ["META-INF"]

  # เพิ่ม gems ที่จำเป็นสำหรับ Rails
  config.gems += ["rails", "puma"]

     
  # รวมไฟล์ที่จำเป็นสำหรับ Rails application
  config.includes = FileList["config.ru"]

  # รวม web.xml สำหรับ servlet container
